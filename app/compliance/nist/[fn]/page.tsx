...
export default function NistFunctionPage() {
  const params = useParams()
  const router = useRouter()
  const fn = typeof params.fn === "string" ? params.fn.toLowerCase() : ""

  const cfg = NIST_FUNCTIONS.find((f) => f.fn === fn)
  ...
  useEffect(() => {
    ...
    const nistControls = store.controls.filter((c) =>
      c.frameworkMappings.some((m) => m.frameworkId === "fw-nist") &&
      c.ref.startsWith((cfg?.prefix ?? "").toUpperCase() + ".")
    )
    setControls(nistControls)
    // Auto-expand first category
    if (cfg && Object.keys(cfg.categories).length > 0) {
  ...
  // Group by category prefix (e.g. GV.OC, GV.RM)
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className={cn("rounded-2xl p-6 text-white bg-gradient-to-br shadow-lg", cfg.gradient)}>
        <button
          onClick={() => router.push("/compliance/nist")}
      ...
      <div className="space-y-3">
        {Object.entries(cfg.categories).map(([cat, catName]) => {
          ...
          return (
            <div
              key={cat}
              className={cn("rounded-xl border overflow-hidden transition-all", cfg.border)}
            >
              {/* Category header */}
              <button
                ...
                className={cn(
                  ...
                  isOpen ? cfg.light : "bg-white hover:bg-gray-50"
                )}
              >
                <div className="flex items-center gap-3">
                  <span className={cn("text-xs font-bold font-mono px-2 py-0.5 rounded", cfg.badge)}>
                    {cat}
                  </span>
                  <div>
                  ...
                  {catControls.length === 0 ? (
                    ...
                    catControls
                      ...
                      .map((ctrl) => (
                        <div
                          key={ctrl.id}
                          className="flex items-start gap-3 px-4 py-3 bg-white hover:bg-gray-50/50 transition group"
                        >
                          {/* Status icon */}
