...
export function hashOTP(otp: string): string {
  return crypto.createHash("sha256").update(otp + (process.env.OTP_SECRET ?? "consent_secret")).digest("hex")
}
...
export async function sendOTPEmail(to: string, otp: string, templateName: string): Promise<boolean> {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
      ...
      <p style="color:#888;font-size:13px">หากคุณไม่ได้ร้องขอ กรุณาเพิกเฉยต่ออีเมลนี้</p>
      <hr style="border:none;border-top:1px solid #eee;margin:24px 0"/>
      <p style="color:#aaa;font-size:11px">ส่งโดยระบบ AI GRC Platform — CyberInno</p>
    </div>
  `
...
export async function sendWithdrawalConfirmEmail(to: string, templateName: string): Promise<void> {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px">
      ...
      <p style="color:#555">ข้อมูลของคุณจะถูกดำเนินการตามนโยบายของเราภายใน 30 วัน</p>
      <hr style="border:none;border-top:1px solid #eee;margin:24px 0"/>
